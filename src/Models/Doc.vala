...
*/
namespace Enigma {
    public class Doc : Object, Json.Serializable {
        public string id { get; set; default = Uuid.string_random (); }
        public string title { get; set; }
        public string subtitle { get; set; }
        public string text { get; set; }
        public static Doc from_json (Json.Node node) requires (node.get_node_type () == OBJECT) {
            return (Doc) Json.gobject_deserialize (typeof (Doc), node);
        }

        public static List<Doc> list_from_json (Json.Node node) requires (node.get_node_type () == ARRAY) {
            var result = new List<Doc> ();

            var json_array = node.get_array ();
            json_array.foreach_element ((_, __, element_node) => {
                result.append (Doc.from_json (element_node));
            });
